...
namespace DDLProxy
{
	...
	public:
		...
		bool SelectServer(_U32 server_id)
		{
			...
			// <_U32> <server_id> <> <>
			if(!DDLProxy<BUF_SIZE>::_Buf.Write(server_id)) return false;
		...
		bool CreateAvatar(const char* avatar_name)
		{
			...
			if(!DDLProxy<BUF_SIZE>::_Buf.WriteData(avatar_name, (unsigned int)sizeof(avatar_name[0])*__length)) return false;
		...
		bool EnterGame(_U32 avatar_id)
		{
			...
			// <_U32> <avatar_id> <> <>
			if(!DDLProxy<BUF_SIZE>::_Buf.Write(avatar_id)) return false;
...
namespace DDLProxy
{
	...
	public:
		...
		bool Buy(const char* shopitem_id)
		{
			...
			if(!DDLProxy<BUF_SIZE>::_Buf.Write(__length)) return false;
			if(!DDLProxy<BUF_SIZE>::_Buf.WriteData(shopitem_id, (unsigned int)sizeof(shopitem_id[0])*__length)) return false;
		...
		bool Discard(const A_UUID& uuid)
		{
			...
			// <A_UUID> <uuid> <> <>
			if(!DDLProxy<BUF_SIZE>::_Buf.Write(uuid)) return false;
		...
		bool Beg(_U32 money)
		{
			...
			// <_U32> <money> <> <>
			if(!DDLProxy<BUF_SIZE>::_Buf.Write(money)) return false;
		...
		bool Gamble(const A_UUID& card, _U32 point)
		{
			...
			// <A_UUID> <card> <> <>
			if(!DDLProxy<BUF_SIZE>::_Buf.Write(card)) return false;
			// <_U32> <point> <> <>
			if(!DDLProxy<BUF_SIZE>::_Buf.Write(point)) return false;
