...
bool CContentExcelImportor::LoadTemplateDefine(const char* filename)
{
	...
	for(size_t ei=0; ei<names.size(); ei++)
	{
		...
		m_enum_map[names[ei]] = ZION_NEW EXCEL_ENUM;
	...
	for(size_t t=0; t<names.size(); t++)
	{
		...
		m_tmpl_map[names[t]] = ZION_NEW EXCEL_TEMPLATE;
		...
		for(int i=0; i<(int)fields.size(); i++)
		{
			...
			{
				Zion::Map<Zion::String, EXCEL_ENUM*>::iterator it;
				it = m_enum_map.find(enu.asCString());
				if(it==m_enum_map.end())
				{
					m_errmsg = Zion::StringFormat("template [%s] fields[%d] enum[%s] not found", names[t].c_str(), i, enu.asCString());
					return false;
				}
				fieldinfo._enum = it->second;
			}
...
void CContentExcelImportor::AddTask(const char* tmpl, const char* filename, const char* sheetname)
{
	TASK_INFO info;
	info.tmpl = tmpl;
...
bool CContentExcelImportor::RunTasks()
{
	...
	{
		if(!ImportSheet(m_Tasks[i].tmpl.c_str(), m_Tasks[i].filename.c_str(), m_Tasks[i].sheetname.c_str()))
		{
			if(m_pXlsFile)
			{
				m_pXlsFile->Quit();
...
bool CContentExcelImportor::ImportSheet(const char* _tmpl, const char* filename, const char* sheetname)
{
	...
	if(m_pXlsFile==NULL || m_XlsFileName!=filename)
	{
		...
		m_pXlsFile = ZION_NEW COLEAutoExcelWrapper();
		if(!m_pXlsFile)
		{
