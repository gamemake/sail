...
void CImportDlg::OnFilePicker(wxFileDirPickerEvent&)
{
	m_checkList->Clear();
...
void CImportDlg::OnSelectAll(wxCommandEvent&)
{
...
bool CImportDlg::ProcessImport()
{
	...
	for(unsigned int i=0; i < m_checkList->GetCount(); ++i)
	{
		...
		m_pImportor->AddTask(strType.c_str(), m_pFilePicker->GetFileName().GetFullPath().ToUTF8(), m_checkList->GetString(i).ToUTF8());
	}
