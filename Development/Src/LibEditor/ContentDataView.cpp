...
void CContentDataView::OnSearch(wxCommandEvent&)
{
	FlashList();
}
...
void CContentDataView::OnObjectAdd(wxCommandEvent&)
{
	if(!CheckModify(false)) return;
...
void CContentDataView::OnObjectSave(wxCommandEvent&)
{
...
void CContentDataView::OnObjectUndo(wxCommandEvent&)
{
	if(!m_pCurInfo || !m_pCurData) return;
	m_pInfo->Set(m_pCurInfo, m_pCurData);
}

void CContentDataView::OnObjectRemove(wxCommandEvent&)
{
...
void CContentDataView::OnObjectReflush(wxCommandEvent&)
{
...
void CContentDataView::OnIdle(wxIdleEvent&)
{
	if(m_nSelectIndex>=0)
	{
...
void CContentDataView::OnMenuCopy(wxCommandEvent&)
{
	if(!m_pCurData) return;
	m_copyUUID = m_pCurData->_uuid;
}

void CContentDataView::OnMenuPaste(wxCommandEvent&)
{
...
void CContentDataView::OnMenuSelAll(wxCommandEvent&)
{
}

void CContentDataView::OnMenuSave(wxCommandEvent&)
{
}
...
void CContentDataView::AppendObject(const DDLReflect::STRUCT_INFO* info, const A_CONTENT_OBJECT* data)
{
	...
	long n = m_pList->InsertItem(m_pList->GetItemCount(), wxString::FromUTF8(data->_name._Value));
