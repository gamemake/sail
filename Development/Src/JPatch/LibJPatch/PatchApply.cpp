...
namespace JPatch
{
	...
	bool ApplyPatch(const char* patch_file, const char* m_path, const char* p_path)
	{
		...
		ctrl_offset = (unsigned int)(*((unsigned int*)(header+8 )));
		data_offset = (unsigned int)(*((unsigned int*)(header+12)));

		fseek(patch, ctrl_offset, SEEK_SET);
			...
			for(;;)
			{
				...
				if(code==OPERATOR_APPEND)
				{
					...
					if(!skip)
					{
						...
						if(bzerror!=BZ_OK)
						{
							printf("error in BZ2_bzReadClose @A %d\n", bzerror);
							goto error;
						}
					}
					continue;
				...
				if(code==OPERATOR_COPY)
				{
					...
					if(!skip)
					{
						...
						while(size>0)
						{
							...
							{
								printf("failed to read source file @C %d\n", bzerror);
								goto error;
							}
							if(fwrite(data, 1, s, tmp_file)!=s)
							{
								printf("failed to write temp file @C %d\n", bzerror);
								goto error;
							}
							size -= s;
						}
