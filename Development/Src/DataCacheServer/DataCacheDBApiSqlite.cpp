...
namespace Zion
{
	namespace DataCache
	{
		...
		private:
			...
			int m_sqlite_delete_user_id;
			int m_sqlite_delete_avatar_scope;
			...
			int m_sqlite_mark_task_avatar_id;
		...
		bool CSqliteDBApi::Connect()
		{
			...
			int table_count = -1;
			sql = "SELECT count(*) FROM sqlite_master WHERE type='table' AND (name='avatar_table' OR name='avatar_object_table' OR name='user_table' OR name='login_history_table' OR name='task_table')";
			if(SQLITE_OK!=sqlite3_exec(m_sqlite, sql, sqlite_callback, &table_count, NULL))
			{
				printf("error in sqlite3_exec() error(%d) : %s\nsql = %s\n", sqlite3_errcode(m_sqlite), sqlite3_errmsg(m_sqlite), sql);
				return false;
			}
			if(table_count==0)
			{
				...
				for(_U32 i=0; sqls[i]!=NULL; i++)
				{
					...
					{
						printf("error in sqlite3_exec() error(%d) : %s\nsql = %s\n", sqlite3_errcode(m_sqlite), sqlite3_errmsg(m_sqlite), sqls[i]);
						return false;
					}
				}
			}
			...
			sql = "INSERT INTO avatar_table(user_id, avatar_scope, avatar_name, avatar_desc) VALUES(:user_id, :avatar_scope, :avatar_name, :avatar_desc)";
			if(SQLITE_OK!=sqlite3_prepare(m_sqlite, sql, -1, &m_sqlite_create, NULL))
			{
				printf("error in sqlite3_prepare() error(%d) : %s\nsql = %s\n", sqlite3_errcode(m_sqlite), sqlite3_errmsg(m_sqlite), sql);
				return false;
			}
			...
			sql = "UPDATE avatar_table SET state = 1 WHERE avatar_id=:avatar_id AND user_id=:user_id AND avatar_scope=:avatar_scope AND state=0";
			if(SQLITE_OK!=sqlite3_prepare(m_sqlite, sql, -1, &m_sqlite_delete, NULL))
			{
				printf("error in sqlite3_prepare() error(%d) : %s\nsql = %s\n", sqlite3_errcode(m_sqlite), sqlite3_errmsg(m_sqlite), sql);
				return false;
			}
			m_sqlite_delete_user_id = sqlite3_bind_parameter_index(m_sqlite_delete, ":user_id");
			m_sqlite_delete_avatar_scope = sqlite3_bind_parameter_index(m_sqlite_delete, ":avatar_scope");
			m_sqlite_delete_avatar_id = sqlite3_bind_parameter_index(m_sqlite_delete, ":avatar_id");

			sql =  "SELECT state, freeze_duetime, avatar_name FROM avatar_table WHERE avatar_id=:avatar_id";
			if(SQLITE_OK!=sqlite3_prepare(m_sqlite, sql, -1, &m_sqlite_check, NULL))
			{
				printf("error in sqlite3_prepare() error(%d) : %s\nsql = %s\n", sqlite3_errcode(m_sqlite), sqlite3_errmsg(m_sqlite), sql);
				return false;
			}
			...
			if(SQLITE_OK!=sqlite3_prepare(m_sqlite, sql, -1, &m_sqlite_load, NULL))
			{
				printf("error in sqlite3_prepare() error(%d) : %s\nsql = %s\n", sqlite3_errcode(m_sqlite), sqlite3_errmsg(m_sqlite), sql);
				return false;
			}
			...
			if(SQLITE_OK!=sqlite3_prepare(m_sqlite, sql, -1, &m_sqlite_insert, NULL))
			{
				printf("error in sqlite3_prepare() error(%d) : %s\nsql = %s\n", sqlite3_errcode(m_sqlite), sqlite3_errmsg(m_sqlite), sql);
				return false;
			}
			...
			if(SQLITE_OK!=sqlite3_prepare(m_sqlite, sql, -1, &m_sqlite_update, NULL))
			{
				printf("error in sqlite3_prepare() error(%d) : %s\nsql = %s\n", sqlite3_errcode(m_sqlite), sqlite3_errmsg(m_sqlite), sql);
				return false;
			}
			...
			if(SQLITE_OK!=sqlite3_prepare(m_sqlite, sql, -1, &m_sqlite_remove, NULL))
			{
				printf("error in sqlite3_prepare() error(%d) : %s\nsql = %s\n", sqlite3_errcode(m_sqlite), sqlite3_errmsg(m_sqlite), sql);
				return false;
			}
			...
			if(SQLITE_OK!=sqlite3_prepare(m_sqlite, sql, -1, &m_sqlite_query, NULL))
			{
				printf("error in sqlite3_prepare() error(%d) : %s\nsql = %s\n", sqlite3_errcode(m_sqlite), sqlite3_errmsg(m_sqlite), sql);
				return false;
			}
			...
			sql = "SELECT state FROM task_table WHERE task_id=:task_id AND avatar_id=:avatar_id";
			if(SQLITE_OK!=sqlite3_prepare(m_sqlite, sql, -1, &m_sqlite_lock_task, NULL))
			{
				printf("error in sqlite3_prepare() error(%d) : %s\nsql = %s\n", sqlite3_errcode(m_sqlite), sqlite3_errmsg(m_sqlite), sql);
				return false;
			}
			...
			sql = "UPDATE task_table SET state=1 WHERE task_id=:task_id AND avatar_id=:avatar_id";
			if(SQLITE_OK!=sqlite3_prepare(m_sqlite, sql, -1, &m_sqlite_mark_task, NULL))
			{
				printf("error in sqlite3_prepare() error(%d) : %s\nsql = %s\n", sqlite3_errcode(m_sqlite), sqlite3_errmsg(m_sqlite), sql);
				return false;
			}
			m_sqlite_mark_task_avatar_id = sqlite3_bind_parameter_index(m_sqlite_mark_task, ":avatar_id");
		...
		bool CSqliteDBApi::DeleteAvatar(_U32 user_id, _U32 avatar_scope, _U32 avatar_id)
		{
			...
			if(SQLITE_OK!=sqlite3_bind_int(m_sqlite_delete, m_sqlite_delete_user_id, (int)user_id))
			{
				printf("error in sqlite3_bind_int(%d), %s\n", sqlite3_errcode(m_sqlite), sqlite3_errmsg(m_sqlite));
				return false;
			}
			if(SQLITE_OK!=sqlite3_bind_int(m_sqlite_delete, m_sqlite_delete_avatar_scope, (int)avatar_scope))
			{
				printf("error in sqlite3_bind_int(%d), %s\n", sqlite3_errcode(m_sqlite), sqlite3_errmsg(m_sqlite));
				return false;
			}
			if(SQLITE_OK!=sqlite3_bind_int(m_sqlite_delete, m_sqlite_delete_avatar_id, (int)avatar_id))
			{
				printf("error in sqlite3_bind_int(%d), %s\n", sqlite3_errcode(m_sqlite), sqlite3_errmsg(m_sqlite));
				return false;
			}
		...
		bool CSqliteDBApi::LoadAvatar(_U32 avatar_id, bool (*callback)(void*, const A_UUID&, const char*, const char*), void* userptr)
		{
			...
			if(!flag)
			{
				//printf("avatar(%u) not found or disable\n", avatar_id);
				return false;
			}
		...
		bool CSqliteDBApi::DeleteTask(_U32 avatar_id, _U32 task_id)
		{
			...
			if(SQLITE_OK!=sqlite3_bind_int(m_sqlite_mark_task, m_sqlite_mark_task_avatar_id, (int)avatar_id))
			{
				printf("error in sqlite3_bind_int(%d), %s\n", sqlite3_errcode(m_sqlite), sqlite3_errmsg(m_sqlite));
				return false;
			}
