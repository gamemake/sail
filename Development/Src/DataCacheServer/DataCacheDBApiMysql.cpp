...
namespace Zion
{
	namespace DataCache
	{
		...
		{
			String sql = StringFormat("UPDATE avatar_table SET state = 1 WHERE avatar_id=%u and user_id=%u and avatar_scope=%u and state=0", avatar_id, user_id, avatar_scope);
			if(mysql_real_query(m_mysql, sql.c_str(), (unsigned long)sql.size())!=0)
			{
				printf("error in mysql_real_query(%d), %s", mysql_errno(m_mysql), mysql_error(m_mysql));
				return false;
		...
		{
			String sql = StringFormat("UPDATE task_table SET state=1 WHERE task_id=%u and avatar_id=%u", task_id, avatar_id);
			if(mysql_real_query(m_mysql, sql.c_str(), (unsigned long)sql.size())!=0)
			{
				printf("error in mysql_real_query(%d), %s", mysql_errno(m_mysql), mysql_error(m_mysql));
				return false;
