...
void connect()
{
	if(uv_tcp_init(uv_default_loop(), &m_socket))
	{
		assert(0);
	}

	if(uv_tcp_connect(&m_connect, &m_socket, (struct sockaddr*)&ip4, &on_connect))
	{
		assert(0);
	}
}
...
void on_connect(uv_connect_t* req, int status)
{
	...
	{
		if(uv_read_start((uv_stream_t*)&m_socket, &on_read_alloc, &on_read_done))
		{
			assert(0);
		}
	}
...
{
	uv_ip4_addr("127.0.0.1", 1980, &ip4);
	connect();
