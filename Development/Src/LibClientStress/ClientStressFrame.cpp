...
CClientStressFrame::CClientStressFrame() : wxFrame(NULL, wxID_ANY, wxT("Client Stress - ")), m_StressLoader(this)
{
	...
	if(pConfig)
	{
		...
		m_FrameData.x = (int)pConfig->Read(wxT("x"), 50);
		m_FrameData.y = (int)pConfig->Read(wxT("y"), 50);
		m_FrameData.w = (int)pConfig->Read(wxT("w"), 350);
		m_FrameData.h = (int)pConfig->Read(wxT("h"), 200);
		Move(m_FrameData.x, m_FrameData.y);
		SetClientSize(m_FrameData.w, m_FrameData.h);
		m_FrameData.m = (int)pConfig->Read(wxT("m"), (long)0);
	}
...
void CClientStressFrame::InitClient()
{
	...
	m_pCmdText = ZION_NEW wxComboBox( pPanel, ID_CMDTEXT, wxT(""), wxDefaultPosition, wxDefaultSize, 0, NULL, wxTE_PROCESS_ENTER|wxCB_DROPDOWN);
...
void CClientStressFrame::OnDebugEnable(wxCommandEvent&)
{
	if(m_pEnableXDebug->IsChecked())
	{
		...
		while(MORequestStatus(request)==MOREQUESTSTATE_PENDING) ZION_SLEEP(1);
...
void CClientStressFrame::OnProtocal(wxCommandEvent&)
{
...
void CClientStressFrame::OnDoCmd(wxCommandEvent&)
{
...
void CClientStressFrame::OnCmdChange(wxCommandEvent&)
{
	Zion::String cmd, data;
	if(_SplitCommand(m_pCmdText->GetValue().ToUTF8(), cmd, data))
	{
...
void CClientStressFrame::OnAddClient(wxCommandEvent& event)
{
	...
	for(int i=0; i<count; i++)
	{
		...
		if(pClient->GetClient()->GetClientConnectionType()=="http")
		{
			...
			pHttp->SetStateCallback(zion_bind(&CClientStressFrame::NotifyHttpState, this, zion_placeholders::_1, zion_placeholders::_2, zion_placeholders::_3));
		}
	}
}

void CClientStressFrame::OnSelectAll(wxCommandEvent&)
{
	unsigned int i;
...
void CClientStressFrame::OnLogin(wxCommandEvent&)
{
...
void CClientStressFrame::OnLogout(wxCommandEvent&)
{
	Zion::Array<_U32> clients;
	GetSelectClients(clients);
	for(size_t i=0; i<clients.size(); i++)
...
void CClientStressFrame::OnAddCase(wxCommandEvent&)
{
...
void CClientStressFrame::OnStressView(wxCommandEvent&)
{
	CStressViewDlg dlg(this);
	dlg.ShowModal();
}

void CClientStressFrame::OnReloadTemplate(wxCommandEvent&)
{
...
void CClientStressFrame::OnOpenSvrParamDlg(wxCommandEvent&)
{
...
void CClientStressFrame::OnTimer(wxTimerEvent&)
{
