...
namespace DDLDataObject
{
	...
	_U32 CConstObject::GetU32(const char* name) const
	{
		...
		{
			return Zion::CNUM_GET(*((A_CONFUSED_U32*)((char*)m_pData + m_Offset + offset)));
		}
	...
	_S32 CConstObject::GetS32(const char* name) const
	{
		...
		{
			return Zion::CNUM_GET(*((A_CONFUSED_S32*)((char*)m_pData + m_Offset + offset)));
		}
	...
	_F32 CConstObject::GetF32(const char* name) const
	{
		...
		if(finfo.type==DDLReflect::TYPE_STRUCT && finfo.sinfo==DDLReflect::GetStruct<A_CONFUSED_S32>())
		{
			return Zion::CNUM_GET(*((A_CONFUSED_F32*)((char*)m_pData + m_Offset + offset)));
		}
	...
	{
		return GetU8(Zion::StringFormat("%s[%d]", name, index).c_str());
	}

	_U16 CConstObject::GetU16(const char* name, _U32 index) const
	{
		return GetU16(Zion::StringFormat("%s[%d]", name, index).c_str());
	}
	...
	{
		return GetS8(Zion::StringFormat("%s[%d]", name, index).c_str());
	}

	_S16 CConstObject::GetS16(const char* name, _U32 index) const
	{
		return GetS16(Zion::StringFormat("%s[%d]", name, index).c_str());
	}
	...
	{
		return GetS64(Zion::StringFormat("%s[%d]", name, index).c_str());
	}

	_F32 CConstObject::GetF32(const char* name, _U32 index) const
	{
		return GetF32(Zion::StringFormat("%s[%d]", name, index).c_str());
	}

	_F64 CConstObject::GetF64(const char* name, _U32 index) const
	{
		return GetF64(Zion::StringFormat("%s[%d]", name, index).c_str());
	}

	const _STR CConstObject::GetSTR(const char* name, _U32 index) const
	{
		return GetSTR(Zion::StringFormat("%s[%d]", name, index).c_str());
	}
	...
	{
		return GetObjPtr(Zion::StringFormat("%s[%d]", name, index).c_str(), type);
	}
	...
	bool CObject::SetU8(const char* name, _U8 val)
	{
		...
		*((_U8*)((char*)m_pData + m_Offset + offset)) = val;
		...
		if(m_pMonitor)
		{
			m_pMonitor->SetDirty(Zion::StringFormat("%s%s%s", m_Name.c_str(), m_Name.empty()?"":".", name).c_str(), m_Offset+offset, sizeof(val));
		}
		return true;
	}
	...
	bool CObject::SetU32(const char* name, _U32 val)
	{
		...
		if(finfo.type==DDLReflect::TYPE_STRUCT && finfo.sinfo==DDLReflect::GetStruct<A_CONFUSED_U32>())
		{
			Zion::CNUM_SET(*((A_CONFUSED_U32*)((char*)m_pData + m_Offset + offset)), val);
			if(m_pMonitor)
			{
				m_pMonitor->SetDirty(Zion::StringFormat("%s%s%s", m_Name.c_str(), m_Name.empty()?"":".", name).c_str(), m_Offset+offset, sizeof(A_CONFUSED_U32));
			}
			return true;
		}
	...
	bool CObject::SetS32(const char* name, _S32 val)
	{
		...
		if(finfo.type==DDLReflect::TYPE_STRUCT && finfo.sinfo==DDLReflect::GetStruct<A_CONFUSED_S32>())
		{
			Zion::CNUM_SET(*((A_CONFUSED_S32*)((char*)m_pData + m_Offset + offset)), val);
			if(m_pMonitor)
			{
				m_pMonitor->SetDirty(Zion::StringFormat("%s%s%s", m_Name.c_str(), m_Name.empty()?"":".", name).c_str(), m_Offset+offset, sizeof(A_CONFUSED_S32));
			}
			return true;
		}
	...
	bool CObject::SetF32(const char* name, _F32 val)
	{
		...
		{
			Zion::CNUM_SET(*((A_CONFUSED_F32*)((char*)m_pData + m_Offset + offset)), val);
			if(m_pMonitor)
			{
				m_pMonitor->SetDirty(Zion::StringFormat("%s%s%s", m_Name.c_str(), m_Name.empty()?"":".", name).c_str(), m_Offset+offset, sizeof(A_CONFUSED_F32));
			}
			return true;
		}
	...
	bool CObject::SetSTR(const char* name, const _STR val)
	{
		...
		if(m_pMonitor)
		{
			m_pMonitor->SetDirty(Zion::StringFormat("%s%s%s", m_Name.c_str(), m_Name.empty()?"":".", name).c_str(), m_Offset+offset, len+1);
		}
		return true;
	}
	...
	bool CObject::SetObjPtr(const char* name, const DDLReflect::STRUCT_INFO* type, const void* val)
	{
		...
		if(m_pMonitor)
		{
			m_pMonitor->SetDirty(Zion::StringFormat("%s%s%s", m_Name.c_str(), m_Name.empty()?"":".", name).c_str(), m_Offset+offset, type->size);
		}
		return true;
	}
	...
	{
		return SetU64(Zion::StringFormat("%s[%d]", name, index).c_str(), val);
	}

	bool CObject::SetS8(const char* name, _U32 index, _S8 val)
	{
		return SetS8(Zion::StringFormat("%s[%d]", name, index).c_str(), val);
	}

	bool CObject::SetS16(const char* name, _U32 index, _S16 val)
	{
		return SetS16(Zion::StringFormat("%s[%d]", name, index).c_str(), val);
	}

	bool CObject::SetS32(const char* name, _U32 index, _S32 val)
	{
		return SetS32(Zion::StringFormat("%s[%d]", name, index).c_str(), val);
	}

	bool CObject::SetS64(const char* name, _U32 index, _S64 val)
	{
		return SetS64(Zion::StringFormat("%s[%d]", name, index).c_str(), val);
	}

	bool CObject::SetF32(const char* name, _U32 index, _F32 val)
	{
		return SetF32(Zion::StringFormat("%s[%d]", name, index).c_str(), val);
	}

	bool CObject::SetF64(const char* name, _U32 index, _F64 val)
	{
		return SetF64(Zion::StringFormat("%s[%d]", name, index).c_str(), val);
	}

	bool CObject::SetSTR(const char* name, _U32 index, const _STR val)
	{
		return SetSTR(Zion::StringFormat("%s[%d]", name, index).c_str(), val);
	}

	bool CObject::SetUUID(const char* name, _U32 index, const A_UUID& val)
	{
		return SetUUID(Zion::StringFormat("%s[%d]", name, index).c_str(), val);
	}

	bool CObject::SetObjPtr(const char* name, _U32 index, const DDLReflect::STRUCT_INFO* type, const void* val)
	{
		return SetObjPtr(Zion::StringFormat("%s[%d]", name, index).c_str(), type, val);
	}
