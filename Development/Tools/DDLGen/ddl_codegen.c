...
int ddlgen_code_task_class_prox(const DDL_CLS* cls, const DDL_TASK* task)
{
	...
	for(f=0; f<cls->funs_count; f++) {
		...
		OutH(0, ")\n");
		OutH(1, "	{\n");
		OutH(1, "		DDLProxy<BUF_SIZE>::_Buf.Reset();\n\n");
		if(need_length) {
		...
		for(a=0; a<cls->funs[f].args_count; a++) {
			...
			if(strcmp(cls->funs[f].args[a].type, "string")==0) {
				if(!cls->funs[f].args[a].size[0] && !cls->funs[f].args[a].count[0]) {
					...
					OutH(3, "if(!DDLProxy<BUF_SIZE>::_Buf.Write(__length)) return false;\n");
					OutH(3, "if(!DDLProxy<BUF_SIZE>::_Buf.WriteData(%s, (unsigned int)sizeof(%s[0])*__length)) return false;\n", cls->funs[f].args[a].name, cls->funs[f].args[a].name);
					continue;
				}
				if(cls->funs[f].args[a].size[0] && !cls->funs[f].args[a].count[0]) {
					OutH(3, "if(!DDLProxy<BUF_SIZE>::_Buf.WriteString(%s)) return false;\n", cls->funs[f].args[a].name);
					continue;
				}
				if(cls->funs[f].args[a].size[0] && cls->funs[f].args[a].count[0]) {
					OutH(3, "	if(!DDLProxy<BUF_SIZE>::_Buf.WriteStringPointer(%s, (_U16)(%s))) return false;\n", cls->funs[f].args[a].name, cls->funs[f].args[a].count);
					continue;
				}
			} else {
				if(cls->funs[f].args[a].count[0]=='\0') {
					OutH(3, "if(!DDLProxy<BUF_SIZE>::_Buf.Write(%s)) return false;\n", cls->funs[f].args[a].name);
					continue;
				} else {
					OutH(3, "__length = (_U16)(%s);\n", cls->funs[f].args[a].count);
					OutH(3, "if(!DDLProxy<BUF_SIZE>::_Buf.Write(__length)) return false;\n");
					OutH(3, "if(!DDLProxy<BUF_SIZE>::_Buf.WritePointer(%s, __length)) return false;\n", cls->funs[f].args[a].name);
					continue;
				}
			}
			OutH(2, "	bu ke neng!!!!!!\n");
		...
		if(cls->funs[f].args_count>0) OutH(0, "\n");
		OutH(1, "		// send\n");
		OutH(1, "		return DDLProxy<BUF_SIZE>::GetClient()->SendData(this->GetClassID(), %d, DDLProxy<BUF_SIZE>::_Buf.GetSize(), DDLProxy<BUF_SIZE>::_Buf.GetBuf());\n", f);
		OutH(1, "	}\n");
	}
	OutH(0, "	};\n");
	OutH(0, "\n");
